...
CALL calculate_RMS_potential_expansion
        ...
        CALL CPU_TIME(tm1)
!
        OPEN (100,file='proba_sym.dat',status='unknown',action='write',position='append')
        OPEN (proba_all_unit,file='proba_sym_all.dat',status='unknown', action='write',position='append')
!
!
! Call for input paramaters reading routine
!------------------------------------------
...
!
	! Direct-only exchange-symmetric S-matrix calculation.
	! ---------------------------------------------------
	! The symmetric open channels are the actual S-matrix channels here:
	! Smat_sym(1:n_sym_open,1:n_sym_open).
			        CALL basic_aux_mat_calcul_sym_direct

	        N_direct_sym = dim_x * nsym

	        CALL potential_mat_calcul_sym_direct
	        CALL make_scatt_mat_sym_direct

	        IF (ALLOCATED(Smat_sym)) DEALLOCATE(Smat_sym, STAT=istatus)
	        ALLOCATE(Smat_sym(1:n_sym_open,1:n_sym_open), STAT=istatus)
	        IF (istatus /= 0) STOP 'Allocation failed for Smat_sym'

	        CALL SolveSMatrixGeneric(N_direct_sym, n_sym_open, mat_M_sym, &
        ...
        IF (ALLOCATED(BAM_x1)) DEALLOCATE(BAM_x1, BAM_x3, BAM_x4, STAT = istatus)
        IF (ALLOCATED(BAM_xx1)) THEN
                DEALLOCATE(BAM_xx1, BAM_xx3, BAM_xx4, &
                           BAM_xx1b, BAM_xx3b, BAM_xx4b, STAT = istatus)
        ENDIF
...
! NEW OPTIMIZED CODE: deallocate each remaining matrix only when allocated.
        ...
        ENDIF
        IF (ALLOCATED(sym_kvec)) THEN
                DEALLOCATE(sym_kvec, STAT = istatus)
        ENDIF
...
!        DEALLOCATE(mat_M, mat_M0, mat_M00, mat_M10, STAT = istatus)
        ...
        PRINT*, 'Direct-only symmetric S-matrix completed at E = ', &
                E*27.211399d0, ' eV'
        PRINT*, 'Cross section accumulation is intentionally disabled here.'
...
!
	...
	!--------------------
	        ...
	        IF (.NOT. ALLOCATED(BAM_x1_sym) .OR. &
	            .NOT. ALLOCATED(BAM_xx1_sym) .OR. &
	            .NOT. ALLOCATED(BAM_xx1b_sym)) THEN
	                PRINT*, 'Error: symmetric basic auxiliary matrices are not allocated.'
	                STOP
	        ENDIF
	                        ...
	                        DO a = 1, sym_ncomp(s)
	                                ...
	                                DO b = 1, sym_ncomp(t)
	                                        ...
	                                        mat_M0_sym(row_sym,os) = mat_M0_sym(row_sym,os) + coeff * ( &
	                                                (-1d0/(2d0*mu_R))*BAM_x1_sym(ir+1,os)*channel_delta &
	                                                - E*BAM_x3_sym(ir+1,os)*channel_delta &
	                                                + E_rot*channel_delta*BAM_x3_sym(ir+1,os) &
	                                                + (1d0/(2d0*mu_R)) * BAM_x4_sym(ir+1,os) * &
	                                                (quant_j1*(quant_j1+1d0) + &
	                                                 quant_j2*(quant_j2+1d0))*channel_delta &
	                                                + (1d0/(2d0*mu_R)) * BAM_x4_sym(ir+1,os) * &
	                                                Wdd(Jtot, quant_j1, quant_k1, quant_j2, quant_k2, &
	                        ...
	                        DO a = 1, sym_ncomp(s)
	                                ...
	                                DO b = 1, sym_ncomp(t)
	                                        ...
	                                        mat_M00_sym(os,ot) = mat_M00_sym(os,ot) + coeff * ( &
	                                                -(1d0/(2d0*mu_R))*BAM_xx1_sym(os,ot)*channel_delta &
	                                                - E*BAM_xx3_sym(os,ot)*channel_delta &
	                                                + E_rot*channel_delta*BAM_xx3_sym(os,ot) &
	                                                + (1d0/(2d0*mu_R)) * BAM_xx4_sym(os,ot) * &
	                                                (quant_j1*(quant_j1+1d0) + &
	                                                 quant_j2*(quant_j2+1d0))*channel_delta &
	                                                + (1d0/(2d0*mu_R))*BAM_xx4_sym(os,ot) * &
	                                                Wdd(Jtot, quant_j1, quant_k1, quant_j2, quant_k2, &
	                                        ...
	                                        mat_M10_sym(os,ot) = mat_M10_sym(os,ot) + coeff * ( &
	                                                -(1d0/(2d0*mu_R))*BAM_xx1b_sym(os,ot)*channel_delta &
	                                                - E*BAM_xx3b_sym(os,ot)*channel_delta &
	                                                + E_rot*channel_delta*BAM_xx3b_sym(os,ot) &
	                                                + (1d0/(2d0*mu_R)) * BAM_xx4b_sym(os,ot) * &
	                                                (quant_j1*(quant_j1+1d0) + &
	                                                 quant_j2*(quant_j2+1d0))*channel_delta &
	                                                + (1d0/(2d0*mu_R))*BAM_xx4b_sym(os,ot) * &
	                                                Wdd(Jtot, quant_j1, quant_k1, quant_j2, quant_k2, &
...
!
	...
	SUBROUTINE cleanup_phase2_matrix_storage(include_sym)
	        ...
	        IF (ALLOCATED(BAM_theta_sym)) DEALLOCATE(BAM_theta_sym, STAT=istatus)
	        IF (ALLOCATED(BAM_x1_sym)) THEN
	                DEALLOCATE(BAM_x1_sym, BAM_x3_sym, BAM_x4_sym, STAT=istatus)
	        ENDIF
	        IF (ALLOCATED(BAM_xx1_sym)) THEN
	                DEALLOCATE(BAM_xx1_sym, BAM_xx3_sym, BAM_xx4_sym, &
	                           BAM_xx1b_sym, BAM_xx3b_sym, BAM_xx4b_sym, &
	                           STAT=istatus)
	        ENDIF
	        IF (ALLOCATED(sym_kvec)) DEALLOCATE(sym_kvec, STAT=istatus)
	...
	SUBROUTINE write_exchange_basis_map
	        ...
	        OPEN(newunit=unit_map, file='direct_sym_channel_map.dat', &
	...
	SUBROUTINE build_exchange_symmetric_open_basis
	        ...
	        ALLOCATE(sym_open_idx(1:n_sym_open), sym_kvec(1:n_sym_open), &
	        ...
	        DO sym_open = 1, n_sym_open
	                ...
	                quant_j1 = sym_quant_mat(1,sym_idx)
	                quant_j2 = sym_quant_mat(3,sym_idx)
	                E_rot = Brot*quant_j1*(quant_j1+1d0) + &
	                        Brot*quant_j2*(quant_j2+1d0)
	                sym_kvec(sym_open) = DSQRT(2d0*mu_R*(E-E_rot))
	        ENDDO

	        PRINT*, 'Exchange-symmetric open channels: old n_open = ', &
	                n_open, ' n_sym_open = ', n_sym_open

	        OPEN(newunit=unit_map, file='direct_sym_open_map.dat', &
	             status='replace', action='write')
	...
	SUBROUTINE write_direct_symmetric_outputs(Smat_sym_in)
	        ...
	        INTEGER :: old_in_open, sym_in_open
	        INTEGER :: old_idx, old_open
	        INTEGER :: os, ot, s_i, s_f
	        INTEGER :: unit_sum
	        REAL(8) :: probability, prob_sum
	        ...
	        WRITE(6,*) 'SYMMETRIC PROBABILITIES at SOME ENERGY'
	        WRITE(6,*) 'incoming sym_open = ', sym_in_open, &
	                ' sym_index = ', sym_open_idx(sym_in_open)
	        WRITE(6,'(10E15.7)') E*27.211399d0, &
	                ((ABS(Smat_sym_in(sym_in_open,os))**2D0), os=1,n_sym_open)

	        WRITE(100,'(16E15.7)') E*27.211399d0, &
	                ((ABS(Smat_sym_in(sym_in_open,os))**2D0), os=1,n_sym_open)

	        DO os = 1, n_sym_open
	                prob_sum = prob_sum + ABS(Smat_sym_in(sym_in_open,os))**2D0
	        ENDDO
	        ...
	        DO os = 1, n_sym_open
	                ...
	                DO ot = 1, n_sym_open
	                        s_f = sym_open_idx(ot)
	                        probability = ABS(Smat_sym_in(os,ot))**2D0

	                        WRITE(101,'(1ES20.10,1X,I5,1X,2I6,1X,4I5,1X,2I6,1X,4I5,1X,1ES20.10)') &
	                                E*27.211399d0, Jtot, &
	                                os, s_i, &
	                                sym_quant_mat(1,s_i), sym_quant_mat(2,s_i), &
	                                sym_quant_mat(3,s_i), sym_quant_mat(4,s_i), &
	                                ot, s_f, &
	                                sym_quant_mat(1,s_f), sym_quant_mat(2,s_f), &
	                                sym_quant_mat(3,s_f), sym_quant_mat(4,s_f), &
	                                probability
	                ENDDO
	        ENDDO

	        OPEN(newunit=unit_sum, file='direct_smatrix_summary.dat', &
	             status='unknown', position='append', action='write')
	        WRITE(unit_sum,'(A)') '# Energy_eV Jtot ncf nsym n_open n_sym_open N_sym sym_in_open prob_sum'
	        WRITE(unit_sum,'(ES20.10,1X,I5,6I8,ES20.10)') &
	                E*27.211399d0, Jtot, ncf, nsym, n_open, n_sym_open, &
	                dim_x*nsym, sym_in_open, prob_sum
	        CLOSE(unit_sum)

	        PRINT*, 'Direct symmetric probability sum = ', prob_sum

	END SUBROUTINE write_direct_symmetric_outputs
