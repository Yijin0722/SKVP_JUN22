                ...
                DO os = 1, n_sym_open
                        ...
                        DO a = 1, sym_ncomp(s)
                                ...
                                DO b = 1, sym_ncomp(t)
                                        ...
                                        old_open_b = find_open_position_sym_direct(old_b)
                                        IF (old_open_b <= 0) CYCLE

                                        coeff = sym_coeff(a,s) * sym_coeff(b,t)

                                        DO ipot = 1, n_pot
	                                                theta_ab = theta_ordered_value(ipot,old_a,old_b)
	                                                mat_M0_sym(row_sym,os) = &
	                                                        mat_M0_sym(row_sym,os) + coeff * &
	                                                        BAM_r0(ipot,ir+1,old_open_b) * &
	                                                        theta_ab
                                        ENDDO
                                ENDDO
                        ENDDO
...
!       ------------------------------------------------------
        DO os = 1, n_sym_open
                ...
                DO ot = 1, n_sym_open
                        ...
                        DO a = 1, sym_ncomp(s)
                                old_a = sym_old_idx(a,s)
                                old_open_a = find_open_position_sym_direct(old_a)
                                ...
                                DO b = 1, sym_ncomp(t)
                                        ...
                                        DO ipot = 1, n_pot
	                                                ...
	                                                mat_M00_sym(os,ot) = mat_M00_sym(os,ot) + coeff * &
	                                                        BAM_r00(ipot,old_open_a,old_open_b) * &
	                                                        theta_ab

	                                                mat_M10_sym(os,ot) = mat_M10_sym(os,ot) + coeff * &
	                                                        BAM_r10(ipot,old_open_a,old_open_b) * &
	                                                        theta_ab
                                        ENDDO
                                ENDDO
                        ENDDO
...
CONTAINS
        ...
        INTEGER FUNCTION find_open_position_sym_direct(old_channel_idx)
                ...
                find_open_position_sym_direct = 0

                DO pos = 1, n_open
                        IF (open_idx(pos) == old_channel_idx) THEN
                                find_open_position_sym_direct = pos
                                RETURN
                        ENDIF
                ENDDO

        END FUNCTION find_open_position_sym_direct
