...
!
	...
	INTEGER, PARAMETER                       :: proba_all_unit = 101
        INTEGER                                  :: N_phase1
        COMPLEX(8), ALLOCATABLE                 :: Smat_ordered_phase1(:,:)
...
!
	! Phase-1 exchange-symmetry validation: solve S matrices only.
	! -------------------------------------------------------------
	! The ordered matrices are still built by the original code path.
	! They are then projected into the exchange-symmetric basis and
	! solved with the same linear algebra used by PhaseShift.
	        CALL basic_aux_mat_calcul
	        CALL potential_mat_calcul
	        CALL make_scatt_mat

	        N_phase1 = dim_x * ncf
	        ALLOCATE(Smat_ordered_phase1(1:n_open,1:n_open), STAT=istatus)
	        IF (istatus /= 0) STOP 'Allocation failed for Smat_ordered_phase1'

	        CALL SolveSMatrixGeneric(N_phase1, n_open, mat_M, mat_M0, &
	                                 mat_M00, mat_M10, Smat_ordered_phase1, &
	                                 'ordered')
	        ...
	        CALL SolveSMatrixGeneric(dim_x*nsym, n_sym_open, mat_M_sym, &
	                                 mat_M0_sym, mat_M00_sym, mat_M10_sym, &
	                                 Smat_sym, 'exchange-symmetric')

	        CALL write_exchange_phase1_outputs(Smat_ordered_phase1, Smat_sym)

	        DEALLOCATE(Smat_ordered_phase1, STAT=istatus)
...
!
	...
	SUBROUTINE write_exchange_basis_map
	        ...
	        OPEN(newunit=unit_map, file='phase1_sym_channel_map.dat', &
	...
	SUBROUTINE build_exchange_symmetric_open_basis
	        ...
	        OPEN(newunit=unit_map, file='phase1_sym_open_map.dat', &
	             status='replace', action='write')
