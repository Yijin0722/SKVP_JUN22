!*********************************************************************************************
!*********************************************************************************************
!*********************************************************************************************
!
SUBROUTINE potential_mat_calcul

        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        use Potential_Interface

        IMPLICIT NONE

        INTEGER :: i, i_prime
        INTEGER :: n, n_prime
        INTEGER :: row, col
        INTEGER :: ipot
        INTEGER :: Ntot

!
!       Total closed-channel matrix dimension
!       -------------------------------------
        Ntot = dim_x * ncf

!
!       Safety check
!       ------------
        IF (.NOT. ALLOCATED(pot_mat)) THEN
                PRINT*, 'Error: pot_mat is not allocated before potential_mat_calcul.'
                PRINT*, 'Call build_potential_index first.'
                STOP
        ENDIF

!
!       Build radial and angular potential BAMs
!       ---------------------------------------
        CALL build_BAM_rs
        CALL build_BAM_thetas

!
!       Allocate matrices
!       -----------------
        IF (ALLOCATED(M_V)) DEALLOCATE(M_V, STAT = istatus)
        ALLOCATE(M_V(1:Ntot,1:Ntot), STAT = istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for M_V in potential_mat_calcul.'
                STOP
        ENDIF
        M_V = 0d0

        IF (ALLOCATED(M0_V)) DEALLOCATE(M0_V, STAT = istatus)
        ALLOCATE(M0_V(1:Ntot,1:n_open), STAT = istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for M0_V in potential_mat_calcul.'
                STOP
        ENDIF
        M0_V = (0d0, 0d0)

        IF (ALLOCATED(M00_V)) DEALLOCATE(M00_V, STAT = istatus)
        ALLOCATE(M00_V(1:n_open,1:n_open), STAT = istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for M00_V in potential_mat_calcul.'
                STOP
        ENDIF
        M00_V = (0d0, 0d0)

        IF (ALLOCATED(M10_V)) DEALLOCATE(M10_V, STAT = istatus)
        ALLOCATE(M10_V(1:n_open,1:n_open), STAT = istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for M10_V in potential_mat_calcul.'
                STOP
        ENDIF
        M10_V = (0d0, 0d0)

!
!       Build M_V: closed-closed potential block
!       ----------------------------------------
        DO i = 1, dim_x
        DO n = 1, ncf

                row = (i-1)*ncf + n

                DO i_prime = 1, dim_x
                DO n_prime = 1, ncf

                        col = (i_prime-1)*ncf + n_prime

                        DO ipot = 1, n_pot
                                M_V(row,col) = M_V(row,col) + &
                                        BAM_r(ipot,i+1,i_prime+1) * &
                                        BAM_theta(ipot,n,n_prime)
                        ENDDO

                ENDDO
                ENDDO

        ENDDO
        ENDDO

!
!       Build M0_V: closed-open potential block
!       ---------------------------------------
        DO i = 1, dim_x
        DO n = 1, ncf

                row = (i-1)*ncf + n

                DO n_prime = 1, n_open

                        DO ipot = 1, n_pot
                                M0_V(row,n_prime) = M0_V(row,n_prime) + &
                                        BAM_r0(ipot,i+1,n_prime) * &
                                        BAM_theta(ipot,n,open_idx(n_prime))
                        ENDDO

                ENDDO

        ENDDO
        ENDDO

!
!       Build M00_V and M10_V: open-open potential blocks
!       --------------------------------------------------
        DO n = 1, n_open
        DO n_prime = 1, n_open

                DO ipot = 1, n_pot

                        M00_V(n,n_prime) = M00_V(n,n_prime) + &
                                BAM_r00(ipot,n,n_prime) * &
                                BAM_theta(ipot,open_idx(n),open_idx(n_prime))

                        M10_V(n,n_prime) = M10_V(n,n_prime) + &
                                BAM_r10(ipot,n,n_prime) * &
                                BAM_theta(ipot,open_idx(n),open_idx(n_prime))

                ENDDO

        ENDDO
        ENDDO

END SUBROUTINE potential_mat_calcul
!
!*********************************************************************************************
!*********************************************************************************************
!*********************************************************************************************

!*********************************************************************************************
!*********************************************************************************************
!*********************************************************************************************
!
SUBROUTINE potential_mat_calcul_sym_direct

        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        use Potential_Interface

        IMPLICIT NONE

        INTEGER :: ir, jr
        INTEGER :: s, t, os, ot
	        INTEGER :: a, b
	        INTEGER :: row_sym, col_sym
	        INTEGER :: ipot
	        INTEGER :: Nsym_dim
	        REAL(8) :: coeff

        Nsym_dim = dim_x * nsym

        IF (.NOT. ALLOCATED(pot_mat)) THEN
                PRINT*, 'Error: pot_mat is not allocated before potential_mat_calcul_sym_direct.'
                PRINT*, 'Call build_potential_index first.'
                STOP
        ENDIF

		        CALL build_BAM_rs_sym_direct
		        CALL build_BAM_theta_sym_direct

        IF (ALLOCATED(mat_M_sym)) DEALLOCATE(mat_M_sym, STAT=istatus)
        IF (ALLOCATED(mat_M0_sym)) DEALLOCATE(mat_M0_sym, STAT=istatus)
        IF (ALLOCATED(mat_M00_sym)) DEALLOCATE(mat_M00_sym, STAT=istatus)
        IF (ALLOCATED(mat_M10_sym)) DEALLOCATE(mat_M10_sym, STAT=istatus)

        ALLOCATE(mat_M_sym(1:Nsym_dim,1:Nsym_dim), &
                 mat_M0_sym(1:Nsym_dim,1:n_sym_open), &
                 mat_M00_sym(1:n_sym_open,1:n_sym_open), &
                 mat_M10_sym(1:n_sym_open,1:n_sym_open), STAT=istatus)
        IF (istatus /= 0) STOP 'Allocation failed for direct symmetric potential matrices'

        mat_M_sym = 0d0
        mat_M0_sym = (0d0,0d0)
        mat_M00_sym = (0d0,0d0)
        mat_M10_sym = (0d0,0d0)

!
!       Closed-closed potential block directly in symmetric basis.
!       ---------------------------------------------------------
        DO ir = 1, dim_x
        DO s = 1, nsym
                row_sym = (ir-1)*nsym + s

                DO jr = 1, dim_x
                DO t = 1, nsym
                        col_sym = (jr-1)*nsym + t

                        DO ipot = 1, n_pot
                                mat_M_sym(row_sym,col_sym) = &
                                        mat_M_sym(row_sym,col_sym) + &
                                        BAM_r(ipot,ir+1,jr+1) * &
                                        BAM_theta_sym(ipot,s,t)
                        ENDDO
                ENDDO
                ENDDO
        ENDDO
        ENDDO

!
!       Closed-open potential block directly in symmetric basis.
!       -------------------------------------------------------
        DO ir = 1, dim_x
        DO s = 1, nsym
                row_sym = (ir-1)*nsym + s

                DO os = 1, n_sym_open
                        t = sym_open_idx(os)

	                        DO ipot = 1, n_pot
	                                mat_M0_sym(row_sym,os) = mat_M0_sym(row_sym,os) + &
	                                        BAM_r0(ipot,ir+1,os) * BAM_theta_sym(ipot,s,t)
	                        ENDDO
	                ENDDO
	        ENDDO
	        ENDDO

!
!       Open-open potential blocks directly in symmetric basis.
!       ------------------------------------------------------
        DO os = 1, n_sym_open
                s = sym_open_idx(os)

                DO ot = 1, n_sym_open
                        t = sym_open_idx(ot)

	                        DO ipot = 1, n_pot
	                                mat_M00_sym(os,ot) = mat_M00_sym(os,ot) + &
	                                        BAM_r00(ipot,os,ot) * BAM_theta_sym(ipot,s,t)

	                                mat_M10_sym(os,ot) = mat_M10_sym(os,ot) + &
	                                        BAM_r10(ipot,os,ot) * BAM_theta_sym(ipot,s,t)
	                        ENDDO
	                ENDDO
	        ENDDO

	        PRINT*, 'Direct symmetric potential matrices built. N = ', Nsym_dim, &
	                ' n_sym_open = ', n_sym_open

	END SUBROUTINE potential_mat_calcul_sym_direct
!
!*********************************************************************************************
!*********************************************************************************************
!*********************************************************************************************

!*********************************************************************************************
!*********************************************************************************************
!*********************************************************************************************
!
SUBROUTINE build_BAM_rs

        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        use Potential_Interface


        IMPLICIT NONE

        INTEGER :: i, i_prime
        INTEGER :: n, n_prime
        INTEGER :: l, ipot
        INTEGER :: q
        REAL(8) :: xm, xr, dx, sxv
        COMPLEX(8) :: sxv0

        CALL read_A_cache(knots_x, gq_root_x, ngqp_x)
!
!       Allocate BAM_r
!       --------------
        IF (ALLOCATED(BAM_r)) THEN
                DEALLOCATE(BAM_r, STAT = istatus)
        ENDIF

        ALLOCATE(BAM_r(1:n_pot,1:pbasst(1)%pb_nbr,1:pbasst(1)%pb_nbr), STAT = istatus)

        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_r in build_BAM_rs.'
                STOP
        ENDIF

        BAM_r = 0d0

        DO i = 1, pbasst(1)%pb_nbr
        DO i_prime = i, MIN(i+pbasst(1)%pb_pa1-1, pbasst(1)%pb_nbr)
        DO q = i_prime, i+pbasst(1)%pb_pa1-1

                xm = 0.5d0 * (knots_x(q+1) + knots_x(q))
                xr = 0.5d0 * (knots_x(q+1) - knots_x(q))

                DO ipot = 1, n_pot
                        sxv = 0d0

                        DO l = 1, ngqp_x/2
                                dx = xr * gq_root_x(l+ngqp_x/2)

                               sxv = sxv + gq_weight_x(l+ngqp_x/2) * &
                                        (A_cache(ipot,q,l,1) * bsp_x(i,xm+dx) * bsp_x(i_prime,xm+dx) + &
                                                A_cache(ipot,q,l,2) * bsp_x(i,xm-dx) * bsp_x(i_prime,xm-dx) )
                        ENDDO

                        BAM_r(ipot,i,i_prime) = BAM_r(ipot,i,i_prime) + &
                                sxv * xr / DSQRT(norm(i)*norm(i_prime))

                        BAM_r(ipot,i_prime,i) = BAM_r(ipot,i,i_prime)
                ENDDO

        ENDDO
        ENDDO
        ENDDO

!
!       Allocate BAM_r0
!       ---------------
!       Important:
!       u0(n,R) is COMPLEX(8), so BAM_r0 must be COMPLEX(8).
!
        IF (ALLOCATED(BAM_r0)) THEN
                DEALLOCATE(BAM_r0, STAT = istatus)
        ENDIF

        ALLOCATE(BAM_r0(1:n_pot,1:pbasst(1)%pb_nbr,1:n_open), STAT = istatus)

        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_r0 in build_BAM_rs.'
                STOP
        ENDIF

        BAM_r0 = (0d0, 0d0)

!
!       Calculate closed-open radial potential matrix:
!
!       BAM_r0(ipot,i,n_prime)
!       =
!       Int dR B_i(R) A_ipot(R) u0_n_prime(R)
!       / sqrt(norm(i))
!
!       u0 already contains:
!       exp(-i*k*R) * h(R) * sqrt(mu_R/k)
!
        BAM_r0 = (0d0, 0d0)

        DO i = 1, pbasst(1)%pb_nbr
        DO n_prime = 1, n_open

                DO q = i, i+pbasst(1)%pb_pa1-1
                        xm = 0.5d0 * (knots_x(q+1) + knots_x(q))
                        xr = 0.5d0 * (knots_x(q+1) - knots_x(q))

                        DO ipot = 1, n_pot
                                sxv0 = (0d0, 0d0)

                                DO l = 1, ngqp_x/2
                                        dx = xr * gq_root_x(l+ngqp_x/2)

                                        sxv0 = sxv0 + gq_weight_x(l+ngqp_x/2) * &
                                                ( bsp_x(i,xm+dx) * A_cache(ipot,q,l,1) * u0(n_prime,xm+dx) + &
                                                bsp_x(i,xm-dx) * A_cache(ipot,q,l,2) * u0(n_prime,xm-dx) )
                                ENDDO

                                BAM_r0(ipot,i,n_prime) = BAM_r0(ipot,i,n_prime) + &
                                        sxv0 * xr / DSQRT(norm(i))
                        ENDDO
                ENDDO

        ENDDO
        ENDDO


        !
!       Allocate BAM_r00 and BAM_r10
!       ----------------------------
        IF (ALLOCATED(BAM_r00)) THEN
                DEALLOCATE(BAM_r00, STAT = istatus)
        ENDIF

        IF (ALLOCATED(BAM_r10)) THEN
                DEALLOCATE(BAM_r10, STAT = istatus)
        ENDIF

        ALLOCATE(BAM_r00(1:n_pot,1:n_open,1:n_open), STAT = istatus)

        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_r00 in build_BAM_rs.'
                STOP
        ENDIF

        ALLOCATE(BAM_r10(1:n_pot,1:n_open,1:n_open), STAT = istatus)

        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_r10 in build_BAM_rs.'
                STOP
        ENDIF

        BAM_r00 = (0d0, 0d0)
        BAM_r10 = (0d0, 0d0)

        DO n = 1, n_open
        DO n_prime = 1, n_open

                DO q = pbasst(1)%pb_pa1, pbasst(1)%pb_nbr
                        xm = 0.5d0 * (knots_x(q+1) + knots_x(q))
                        xr = 0.5d0 * (knots_x(q+1) - knots_x(q))

                        DO ipot = 1, n_pot
                                sxv0 = (0d0, 0d0)

                                DO l = 1, ngqp_x/2
                                        dx = xr * gq_root_x(l+ngqp_x/2)

                                        sxv0 = sxv0 + gq_weight_x(l+ngqp_x/2) * &
                                                ( u0(n,xm+dx) * A_cache(ipot,q,l,1) * u0(n_prime,xm+dx) + &
                                                        u0(n,xm-dx) * A_cache(ipot,q,l,2) * u0(n_prime,xm-dx) )
                                ENDDO

                                BAM_r00(ipot,n,n_prime) = BAM_r00(ipot,n,n_prime) + sxv0 * xr

                                sxv0 = (0d0, 0d0)

                                DO l = 1, ngqp_x/2
                                        dx = xr * gq_root_x(l+ngqp_x/2)

                                        sxv0 = sxv0 + gq_weight_x(l+ngqp_x/2) * &
                                                        ( CONJG(u0(n,xm+dx)) * A_cache(ipot,q,l,1) * u0(n_prime,xm+dx) + &
                                                        CONJG(u0(n,xm-dx)) * A_cache(ipot,q,l,2) * u0(n_prime,xm-dx) )
                                ENDDO

                                BAM_r10(ipot,n,n_prime) = BAM_r10(ipot,n,n_prime) + sxv0 * xr
                        ENDDO
                ENDDO

        ENDDO
        ENDDO

END SUBROUTINE build_BAM_rs
!
!*********************************************************************************************
!=============================================================================================

SUBROUTINE build_BAM_rs_sym_direct
!
!*********************************************************************************************
!
! Direct exchange-symmetric radial potential matrices.
!
! The closed-closed block BAM_r is unchanged. The open radial blocks reuse
! the original variable names, but are allocated in the symmetric open basis:
!
!   BAM_r0  (n_pot, pb_nbr,     n_sym_open)
!   BAM_r00 (n_pot, n_sym_open, n_sym_open)
!   BAM_r10 (n_pot, n_sym_open, n_sym_open)
!
!=============================================================================================
!
        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        USE Potential_Interface

        IMPLICIT NONE

        INTEGER :: i, i_prime
        INTEGER :: n, n_prime
        INTEGER :: l, ipot
        INTEGER :: q, unit_sum
        INTEGER(8) :: old_r0_entries, sym_r0_entries
        INTEGER(8) :: old_roo_entries, sym_roo_entries
        REAL(8) :: xm, xr, dx, sxv
        REAL(8) :: rp, rm, k_n, k_np
        COMPLEX(8) :: sxv0
        COMPLEX(8) :: u0_n_p, u0_n_m, u0_np_p, u0_np_m

        CALL read_A_cache(knots_x, gq_root_x, ngqp_x)

        IF (.NOT. ALLOCATED(sym_kvec)) THEN
                PRINT*, 'Error: sym_kvec is not allocated before build_BAM_rs_sym_direct.'
                STOP
        ENDIF

!
!       Closed-closed radial potential block.
!       ------------------------------------
        IF (ALLOCATED(BAM_r)) THEN
                DEALLOCATE(BAM_r, STAT=istatus)
        ENDIF

        ALLOCATE(BAM_r(1:n_pot,1:pbasst(1)%pb_nbr,1:pbasst(1)%pb_nbr), &
                 STAT=istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_r in build_BAM_rs_sym_direct.'
                STOP
        ENDIF

        BAM_r = 0d0

        DO i = 1, pbasst(1)%pb_nbr
        DO i_prime = i, MIN(i+pbasst(1)%pb_pa1-1, pbasst(1)%pb_nbr)
        DO q = i_prime, i+pbasst(1)%pb_pa1-1

                xm = 0.5d0 * (knots_x(q+1) + knots_x(q))
                xr = 0.5d0 * (knots_x(q+1) - knots_x(q))

                DO ipot = 1, n_pot
                        sxv = 0d0

                        DO l = 1, ngqp_x/2
                                dx = xr * gq_root_x(l+ngqp_x/2)

                                sxv = sxv + gq_weight_x(l+ngqp_x/2) * &
                                        (A_cache(ipot,q,l,1) * &
                                         bsp_x(i,xm+dx) * bsp_x(i_prime,xm+dx) + &
                                         A_cache(ipot,q,l,2) * &
                                         bsp_x(i,xm-dx) * bsp_x(i_prime,xm-dx))
                        ENDDO

                        BAM_r(ipot,i,i_prime) = BAM_r(ipot,i,i_prime) + &
                                sxv * xr / DSQRT(norm(i)*norm(i_prime))

                        BAM_r(ipot,i_prime,i) = BAM_r(ipot,i,i_prime)
                ENDDO

        ENDDO
        ENDDO
        ENDDO

!
!       Closed-open radial potential block in symmetric open basis.
!       ----------------------------------------------------------
        IF (ALLOCATED(BAM_r0)) THEN
                DEALLOCATE(BAM_r0, STAT=istatus)
        ENDIF

        ALLOCATE(BAM_r0(1:n_pot,1:pbasst(1)%pb_nbr,1:n_sym_open), &
                 STAT=istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for symmetric BAM_r0.'
                STOP
        ENDIF

        BAM_r0 = (0d0,0d0)

        DO i = 1, pbasst(1)%pb_nbr
        DO n_prime = 1, n_sym_open
                k_np = sym_kvec(n_prime)

                DO q = i, i+pbasst(1)%pb_pa1-1
                        xm = 0.5d0 * (knots_x(q+1) + knots_x(q))
                        xr = 0.5d0 * (knots_x(q+1) - knots_x(q))

                        DO ipot = 1, n_pot
                                sxv0 = (0d0,0d0)

                                DO l = 1, ngqp_x/2
                                        dx = xr * gq_root_x(l+ngqp_x/2)
                                        rp = xm + dx
                                        rm = xm - dx

                                        u0_np_p = ZEXP(-(0d0,1d0)*k_np*rp) * &
                                                h(rp) * DSQRT(mu_R/k_np)
                                        u0_np_m = ZEXP(-(0d0,1d0)*k_np*rm) * &
                                                h(rm) * DSQRT(mu_R/k_np)

                                        sxv0 = sxv0 + gq_weight_x(l+ngqp_x/2) * &
                                                (bsp_x(i,rp) * A_cache(ipot,q,l,1) * u0_np_p + &
                                                 bsp_x(i,rm) * A_cache(ipot,q,l,2) * u0_np_m)
                                ENDDO

                                BAM_r0(ipot,i,n_prime) = BAM_r0(ipot,i,n_prime) + &
                                        sxv0 * xr / DSQRT(norm(i))
                        ENDDO
                ENDDO

        ENDDO
        ENDDO

!
!       Open-open radial potential blocks in symmetric open basis.
!       ---------------------------------------------------------
        IF (ALLOCATED(BAM_r00)) THEN
                DEALLOCATE(BAM_r00, STAT=istatus)
        ENDIF

        IF (ALLOCATED(BAM_r10)) THEN
                DEALLOCATE(BAM_r10, STAT=istatus)
        ENDIF

        ALLOCATE(BAM_r00(1:n_pot,1:n_sym_open,1:n_sym_open), STAT=istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for symmetric BAM_r00.'
                STOP
        ENDIF

        ALLOCATE(BAM_r10(1:n_pot,1:n_sym_open,1:n_sym_open), STAT=istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for symmetric BAM_r10.'
                STOP
        ENDIF

        BAM_r00 = (0d0,0d0)
        BAM_r10 = (0d0,0d0)

        DO n = 1, n_sym_open
        DO n_prime = 1, n_sym_open
                k_n = sym_kvec(n)
                k_np = sym_kvec(n_prime)

                DO q = pbasst(1)%pb_pa1, pbasst(1)%pb_nbr
                        xm = 0.5d0 * (knots_x(q+1) + knots_x(q))
                        xr = 0.5d0 * (knots_x(q+1) - knots_x(q))

                        DO ipot = 1, n_pot
                                sxv0 = (0d0,0d0)

                                DO l = 1, ngqp_x/2
                                        dx = xr * gq_root_x(l+ngqp_x/2)
                                        rp = xm + dx
                                        rm = xm - dx

                                        u0_n_p = ZEXP(-(0d0,1d0)*k_n*rp) * &
                                                h(rp) * DSQRT(mu_R/k_n)
                                        u0_n_m = ZEXP(-(0d0,1d0)*k_n*rm) * &
                                                h(rm) * DSQRT(mu_R/k_n)
                                        u0_np_p = ZEXP(-(0d0,1d0)*k_np*rp) * &
                                                h(rp) * DSQRT(mu_R/k_np)
                                        u0_np_m = ZEXP(-(0d0,1d0)*k_np*rm) * &
                                                h(rm) * DSQRT(mu_R/k_np)

                                        sxv0 = sxv0 + gq_weight_x(l+ngqp_x/2) * &
                                                (u0_n_p * A_cache(ipot,q,l,1) * u0_np_p + &
                                                 u0_n_m * A_cache(ipot,q,l,2) * u0_np_m)
                                ENDDO

                                BAM_r00(ipot,n,n_prime) = &
                                        BAM_r00(ipot,n,n_prime) + sxv0 * xr

                                sxv0 = (0d0,0d0)

                                DO l = 1, ngqp_x/2
                                        dx = xr * gq_root_x(l+ngqp_x/2)
                                        rp = xm + dx
                                        rm = xm - dx

                                        u0_n_p = ZEXP(-(0d0,1d0)*k_n*rp) * &
                                                h(rp) * DSQRT(mu_R/k_n)
                                        u0_n_m = ZEXP(-(0d0,1d0)*k_n*rm) * &
                                                h(rm) * DSQRT(mu_R/k_n)
                                        u0_np_p = ZEXP(-(0d0,1d0)*k_np*rp) * &
                                                h(rp) * DSQRT(mu_R/k_np)
                                        u0_np_m = ZEXP(-(0d0,1d0)*k_np*rm) * &
                                                h(rm) * DSQRT(mu_R/k_np)

                                        sxv0 = sxv0 + gq_weight_x(l+ngqp_x/2) * &
                                                (CONJG(u0_n_p) * A_cache(ipot,q,l,1) * u0_np_p + &
                                                 CONJG(u0_n_m) * A_cache(ipot,q,l,2) * u0_np_m)
                                ENDDO

                                BAM_r10(ipot,n,n_prime) = &
                                        BAM_r10(ipot,n,n_prime) + sxv0 * xr
                        ENDDO
                ENDDO

        ENDDO
        ENDDO

        old_r0_entries = INT(n_pot,8) * INT(pbasst(1)%pb_nbr,8) * INT(n_open,8)
        sym_r0_entries = INT(n_pot,8) * INT(pbasst(1)%pb_nbr,8) * &
                INT(n_sym_open,8)
        old_roo_entries = 2_8 * INT(n_pot,8) * INT(n_open,8) * INT(n_open,8)
        sym_roo_entries = 2_8 * INT(n_pot,8) * INT(n_sym_open,8) * &
                INT(n_sym_open,8)

        OPEN(newunit=unit_sum, file='radial_sym_summary.dat', &
             status='unknown', position='append', action='write')
        WRITE(unit_sum,'(A)') &
                '# Energy_eV Jtot n_open n_sym_open old_r0 sym_r0 ratio_r0 old_r0010 sym_r0010 ratio_r0010'
        WRITE(unit_sum,'(ES20.10,1X,I5,2I8,2I14,ES20.10,2I14,ES20.10)') &
                E*27.211399d0, Jtot, n_open, n_sym_open, &
                old_r0_entries, sym_r0_entries, &
                DBLE(sym_r0_entries)/DBLE(old_r0_entries), &
                old_roo_entries, sym_roo_entries, &
                DBLE(sym_roo_entries)/DBLE(old_roo_entries)
        CLOSE(unit_sum)

        PRINT*, 'Direct symmetric radial potential matrices built.'
        PRINT*, 'BAM_r0 old entries = ', old_r0_entries, &
                ' sym entries = ', sym_r0_entries
        PRINT*, 'BAM_r00/BAM_r10 old entries = ', old_roo_entries, &
                ' sym entries = ', sym_roo_entries

END SUBROUTINE build_BAM_rs_sym_direct
!
!*********************************************************************************************
!=============================================================================================

	!*********************************************************************************************
	!*********************************************************************************************
!*********************************************************************************************
!
SUBROUTINE build_BAM_thetas
!
!*********************************************************************************************
!
! This subroutine builds the angular potential matrix:
!
!   BAM_theta(ipot,n,n_prime)
!
! where
!
!   ipot <----> (lambda1, lambda2, m)
!   n    <----> (j1, k1, j2, k2)
!   n'   <----> (j1', k1', j2', k2')
!
!=============================================================================================
!
        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        use Potential_Interface

!
        IMPLICIT NONE
!
        INTEGER :: ipot, n, n_prime
        INTEGER :: lambda1, lambda2, m
        INTEGER :: j1, k1, j2, k2
        INTEGER :: j1_prime, k1_prime, j2_prime, k2_prime
        REAL(8) :: G1p, G2p, G1m, G2m, basis_factor
!

      
!       Allocate BAM_theta
!       ------------------
        IF (ALLOCATED(BAM_theta)) THEN
                DEALLOCATE(BAM_theta, STAT = istatus)
        ENDIF

        ALLOCATE(BAM_theta(1:n_pot,1:ncf,1:ncf), STAT = istatus)

        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_theta in build_BAM_theta.'
                STOP
        ENDIF

        BAM_theta = 0d0
!
!       Calculate BAM_theta(ipot,n,n_prime)
!       -----------------------------------
        DO ipot = 1, n_pot

                        lambda1 = pot_mat(1,ipot)
                        lambda2 = pot_mat(2,ipot)
                        m       = pot_mat(3,ipot)


                DO n = 1, ncf

                        j1 = quant_mat(1,n)
                        k1 = quant_mat(2,n)
                        j2 = quant_mat(3,n)
                        k2 = quant_mat(4,n)

                        DO n_prime = 1, ncf

                                j1_prime = quant_mat(1,n_prime)
                                k1_prime = quant_mat(2,n_prime)
                                j2_prime = quant_mat(3,n_prime)
                                k2_prime = quant_mat(4,n_prime)

                                G1p = gaunt_coeff(j1,k1,lambda1, m,j1_prime,k1_prime)
                                G2p = gaunt_coeff(j2,k2,lambda2,-m,j2_prime,k2_prime)

                                G1m = gaunt_coeff(j1,k1,lambda1,-m,j1_prime,k1_prime)
                                G2m = gaunt_coeff(j2,k2,lambda2, m,j2_prime,k2_prime)

                                IF (m == 0) THEN
                                basis_factor = DSQRT(2d0*pi)
                                ELSE
                                basis_factor = (-1d0)**m * 2d0 * DSQRT(pi)
                                ENDIF

                                BAM_theta(ipot,n,n_prime) = basis_factor * 0.5d0 * (G1p*G2p + G1m*G2m)

                        ENDDO
                ENDDO
        ENDDO
!
!=============================================================================================
!
END SUBROUTINE build_BAM_thetas
!
!*********************************************************************************************
!=============================================================================================

REAL(8) FUNCTION theta_ordered_value(ipot,n,n_prime)
!
!*********************************************************************************************
!
! Compute one ordered-channel angular potential element without storing the
! full BAM_theta(n_pot,ncf,ncf) matrix.
!
!=============================================================================================
!
        USE generateparameters
        USE AtomDiatomskvp
        USE Potential_Interface

        IMPLICIT NONE

        INTEGER, INTENT(IN) :: ipot, n, n_prime
        INTEGER :: lambda1, lambda2, m
        INTEGER :: j1, k1, j2, k2
        INTEGER :: j1_prime, k1_prime, j2_prime, k2_prime
        REAL(8) :: G1p, G2p, G1m, G2m, basis_factor

        lambda1 = pot_mat(1,ipot)
        lambda2 = pot_mat(2,ipot)
        m       = pot_mat(3,ipot)

        j1 = quant_mat(1,n)
        k1 = quant_mat(2,n)
        j2 = quant_mat(3,n)
        k2 = quant_mat(4,n)

        j1_prime = quant_mat(1,n_prime)
        k1_prime = quant_mat(2,n_prime)
        j2_prime = quant_mat(3,n_prime)
        k2_prime = quant_mat(4,n_prime)

        G1p = gaunt_coeff(j1,k1,lambda1, m,j1_prime,k1_prime)
        G2p = gaunt_coeff(j2,k2,lambda2,-m,j2_prime,k2_prime)

        G1m = gaunt_coeff(j1,k1,lambda1,-m,j1_prime,k1_prime)
        G2m = gaunt_coeff(j2,k2,lambda2, m,j2_prime,k2_prime)

        IF (m == 0) THEN
                basis_factor = DSQRT(2d0*pi)
        ELSE
                basis_factor = (-1d0)**m * 2d0 * DSQRT(pi)
        ENDIF

        theta_ordered_value = basis_factor * 0.5d0 * (G1p*G2p + G1m*G2m)

END FUNCTION theta_ordered_value
!
!*********************************************************************************************
!=============================================================================================

SUBROUTINE build_BAM_theta_sym_direct
!
!*********************************************************************************************
!
! Build the angular potential matrix directly in the monomer-exchange
! symmetric basis. No ordered BAM_theta(n_pot,ncf,ncf) allocation is needed.
!
!=============================================================================================
!
        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        USE Potential_Interface

        IMPLICIT NONE

        INTEGER :: ipot, s, t
        INTEGER :: a, b
        INTEGER :: old_a, old_b
        INTEGER :: unit_sum
        INTEGER(8) :: old_entries, sym_entries
        REAL(8) :: coeff, ratio
        REAL(8) :: theta_ordered_value

        IF (ALLOCATED(BAM_theta_sym)) THEN
                DEALLOCATE(BAM_theta_sym, STAT=istatus)
        ENDIF

        ALLOCATE(BAM_theta_sym(1:n_pot,1:nsym,1:nsym), STAT=istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for direct BAM_theta_sym.'
                STOP
        ENDIF

        BAM_theta_sym = 0d0

        DO ipot = 1, n_pot
        DO s = 1, nsym
        DO t = 1, nsym
                DO a = 1, sym_ncomp(s)
                        old_a = sym_old_idx(a,s)
                        DO b = 1, sym_ncomp(t)
                                old_b = sym_old_idx(b,t)
                                coeff = sym_coeff(a,s) * sym_coeff(b,t)
                                BAM_theta_sym(ipot,s,t) = BAM_theta_sym(ipot,s,t) + &
                                        coeff * theta_ordered_value(ipot,old_a,old_b)
                        ENDDO
                ENDDO
        ENDDO
        ENDDO
        ENDDO

        old_entries = INT(n_pot,8) * INT(ncf,8) * INT(ncf,8)
        sym_entries = INT(n_pot,8) * INT(nsym,8) * INT(nsym,8)
        ratio = DBLE(sym_entries) / DBLE(old_entries)

        OPEN(newunit=unit_sum, file='bamtheta_sym_summary.dat', &
             status='unknown', position='append', action='write')
        WRITE(unit_sum,'(A)') '# Energy_eV Jtot n_pot ncf nsym old_entries sym_entries ratio mode'
        WRITE(unit_sum,'(ES20.10,1X,I5,3I8,2I14,ES20.10,1X,A)') &
                E*27.211399d0, Jtot, n_pot, ncf, nsym, &
                old_entries, sym_entries, ratio, 'direct'
        CLOSE(unit_sum)

        PRINT*, 'Built BAM_theta directly in symmetric basis.'
        PRINT*, 'BAM_theta old entries avoided = ', old_entries, &
                ' sym entries = ', sym_entries, ' ratio = ', ratio

END SUBROUTINE build_BAM_theta_sym_direct
!
!*********************************************************************************************
!=============================================================================================

	!*********************************************************************************************
	!*********************************************************************************************
!*********************************************************************************************
!
SUBROUTINE build_BAM_theta_sym_from_old
!
!*********************************************************************************************
!
! Project the ordered-channel angular potential matrix into the symmetric
! monomer-exchange basis:
!
!   BAM_theta_sym(ipot,s,t)
!     = sum_a sum_b c(a,s)c(b,t) BAM_theta(ipot,old_a,old_b)
!
! This validation-stage routine still requires the ordered BAM_theta to exist.
!
!=============================================================================================
!
        USE generateparameters
        USE AtomDiatomskvp
        USE omp_lib
        use Potential_Interface

        IMPLICIT NONE

        INTEGER :: ipot, s, t
        INTEGER :: a, b
        INTEGER :: old_a, old_b
        INTEGER :: unit_sum
        INTEGER(8) :: old_entries, sym_entries
        REAL(8) :: coeff, ratio

        IF (.NOT. ALLOCATED(BAM_theta)) THEN
                PRINT*, 'Error: BAM_theta is not allocated before build_BAM_theta_sym_from_old.'
                STOP
        ENDIF

        IF (ALLOCATED(BAM_theta_sym)) THEN
                DEALLOCATE(BAM_theta_sym, STAT=istatus)
        ENDIF

        ALLOCATE(BAM_theta_sym(1:n_pot,1:nsym,1:nsym), STAT=istatus)
        IF (istatus /= 0) THEN
                PRINT*, 'Error: allocation failed for BAM_theta_sym.'
                STOP
        ENDIF

        BAM_theta_sym = 0d0

        DO ipot = 1, n_pot
        DO s = 1, nsym
        DO t = 1, nsym
                DO a = 1, sym_ncomp(s)
                        old_a = sym_old_idx(a,s)
                        DO b = 1, sym_ncomp(t)
                                old_b = sym_old_idx(b,t)
                                coeff = sym_coeff(a,s) * sym_coeff(b,t)
                                BAM_theta_sym(ipot,s,t) = BAM_theta_sym(ipot,s,t) + &
                                        coeff * BAM_theta(ipot,old_a,old_b)
                        ENDDO
                ENDDO
        ENDDO
        ENDDO
        ENDDO

        old_entries = INT(n_pot,8) * INT(ncf,8) * INT(ncf,8)
        sym_entries = INT(n_pot,8) * INT(nsym,8) * INT(nsym,8)
        ratio = DBLE(sym_entries) / DBLE(old_entries)

        OPEN(newunit=unit_sum, file='bamtheta_sym_summary.dat', &
             status='unknown', position='append', action='write')
        WRITE(unit_sum,'(A)') '# Energy_eV Jtot n_pot ncf nsym old_entries sym_entries ratio'
        WRITE(unit_sum,'(ES20.10,1X,I5,3I8,2I14,ES20.10)') &
                E*27.211399d0, Jtot, n_pot, ncf, nsym, &
                old_entries, sym_entries, ratio
        CLOSE(unit_sum)

        PRINT*, 'Projected BAM_theta to symmetric basis.'
        PRINT*, 'BAM_theta old entries = ', old_entries, &
                ' sym entries = ', sym_entries, ' ratio = ', ratio

END SUBROUTINE build_BAM_theta_sym_from_old
!
!*********************************************************************************************
!=============================================================================================
